...
const button0 = (status) => {
   ...
   ()=>{
       ...
       if(click==1){
        //    getId.innerHTML = '0';
        columnValue ? `${  getId.innerHTML = `${columnValue}`}` : `${  getId.innerHTML = '0'}`;
           //store value into the array
           valueArray[column] = '0';
           // style value
           const buttonValue = '0';
           document.getElementById(columnId).setAttribute('style',`font-size:40px; color:  ${columnValue === buttonValue || columnValue === '' ? '#d400d4' : 'chartreuse'};display:flex; justify-content: center;align-items: center;`);
           const status = check3Column();
           status ?  buttonX(!true) :  buttonX(true);
   ...
   ()=>{
       ...
       let click=0;
        ...
        if(click==1){
            //getId.innerHTML = 'x';
            columnValue ? `${  getId.innerHTML = `${columnValue}`}` : `${  getId.innerHTML = 'x'}`;
             ...
             //store value into the array
               valueArray[column] = 'x';
                // style value
                const buttonValue = 'x';
               document.getElementById(columnId).setAttribute('style',`font-size:40px;color: ${columnValue === buttonValue || columnValue === '' ? 'chartreuse': '#d400d4' };display:flex; justify-content: center;align-items: center;`);
               const status = check3Column();
               status ?  button0(!true) :  button0(!false);
        }
        else{
            columnValue ? `${  getId.innerHTML = `${columnValue}`}` : `${  getId.innerHTML = ''}`;
        }
